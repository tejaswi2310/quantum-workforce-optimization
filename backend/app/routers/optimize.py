...
def start_optimization(project_id: str, run_type: str, parameters: dict, background_tasks: BackgroundTasks, db: Session, current_user: User):
    ...
    background_tasks.add_task(run_optimization, str(opt_run.id))

    return opt_run

@router.post("/classical", response_model=OptimizationRunResponse)
def optimize_classical(project_id: str, req: OptimizationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def optimize_shifts(project_id: str, req: OptimizationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def optimize_quantum(project_id: str, req: OptimizationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def optimize_hybrid(project_id: str, req: OptimizationRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
...
def get_optimization_runs(project_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    project = db.query(Project).filter(Project.id == project_id, Project.user_id == current_user.id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
def get_optimization_run(project_id: str, run_id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
