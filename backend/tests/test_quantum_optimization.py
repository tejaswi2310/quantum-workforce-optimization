...
def temp_workspace(tmp_path, monkeypatch):
    ...
    def mock_result_path(self, filename):
        import os
        from pathlib import Path
        # Some calls might pass full paths or relative.
        return results_dir / filename

    def mock_data_path(self, filename):
        from pathlib import Path
        return (tmp_path / "data") / filename

    def mock_ensure_run_dirs(self):
        pass

    monkeypatch.setattr('app.services.storage_service.StorageService.result_path', mock_result_path)
    monkeypatch.setattr('app.services.storage_service.StorageService.data_path', mock_data_path)
    monkeypatch.setattr('app.services.storage_service.StorageService.ensure_run_dirs', mock_ensure_run_dirs)
