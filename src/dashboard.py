...
def calculate_kpis(vol_mult, df_c, df_v):
    ...
    # Calculate costs
    opt_agents = int(np.ceil(df_c['scheduled_agents'].sum() * vol_mult))
    total_cost = opt_agents * 15
    
    # Utilization and Idle
    required = np.ceil(df_c['required_agents'].sum() * vol_mult)
    scheduled = np.ceil(df_c['scheduled_agents'].sum() * vol_mult)
    idle_time = max(0, scheduled - required)
    utilization = (required / scheduled * 100) if scheduled > 0 else 100
    
    # Overtime (mocked as 0 for optimization, could derive from slider buffer)
    overtime = 0 
    
    # Wait time and SLA (from Erlang simulation)
    # adjust SLA downwards heavily if volume increases beyond coverage
    coverage_ratio = scheduled / (required + 0.001)
    effective_sla = df_v['sla_percent'].mean()
    if vol_mult > 1.0:
        effective_sla = max(0, effective_sla - ((vol_mult - 1.0) * 40)) # Degradation penalty if we didn't re-optimize
        
    avg_wait = df_v['asa_seconds'].mean() if 'asa_seconds' in df_v.columns else 15.0
    if vol_mult > 1.0:
        avg_wait = min(300, avg_wait * vol_mult * 1.5)
        
    queue_length = max(0, int((vol_mult - 1.0) * 10))
...
with tabs[1]:
    ...
    if df_fc_filtered is not None and not df_fc_filtered.empty:
        ...
        df_fc_filtered['lower_bound'] = df_fc_filtered['lower_bound'] * volume_multiplier
        df_fc_filtered['upper_bound'] = df_fc_filtered['upper_bound'] * volume_multiplier
...
with tabs[5]:
    ...
    if df_validation is not None and not df_validation.empty:
        ...
        if volume_multiplier > 1.0:
            val_df['sla_percent'] = val_df['sla_percent'].apply(lambda x: max(0, x - ((volume_multiplier - 1.0) * 40)))
